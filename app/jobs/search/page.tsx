  ...
  useEffect(() => {
    if (isAuthenticated) {
      fetch("/api/jobs/save")
        ...
        .then((data) => {
          if (data.success) {
            const saved = new Set<string>(data.savedJobs.map((sj: any) => sj.job._id?.toString() || sj.job.toString()));
            setSavedJobs(saved);
          }
        })
          ...
          if (data.success) {
            ...
            if (isAuthenticated && data.jobs) {
              ...
              Promise.all(appliedChecks).then((results) => {
                const applied = new Set<string>(results.filter((r) => r.applied).map((r) => r.jobId));
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-6 sm:py-8">
        ...
        {showMobileFilters && (
          <div className="lg:hidden mb-6 bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
                      ...
                      try {
                        const response = await fetch("/api/jobs/alerts", {
                          ...
                          body: JSON.stringify({
                            ...
                            remote: employmentType.includes("Flexible schedule"),
                          }),
                        });
                        if (response.ok) {
                          alert("Job alert created! You'll receive email notifications when new matching jobs are posted.");
                        }
                      } catch (error) {
